...
cleanup () {
    kubectl exec exp-pod -n $namespace -- rm /data/launch -rf
    kubectl exec exp-pod -n $namespace -- rm /data/output -rf
    kubectl exec exp-pod -n $namespace -- rm /data/work -rf
    rm -r $cluster
...
do
    ...
    kubectl apply -f "loadToCache/$workflow.yaml"
    kubectl rollout status daemonset workflow-prepare -n $namespace
    kubectl delete -f "loadToCache/$workflow.yaml" --wait
    trial=1
    ...
    do
        ...
        do
            ...
            do
                ...
                if [ ! -d "$experiment" ]; then
                    echo "workflow: $workflow strategy: $strategy"
                    kubectl delete -f workflow-scheduler.yaml  --wait
                    kubectl apply -f workflow-scheduler.yaml
                    sleep 10
